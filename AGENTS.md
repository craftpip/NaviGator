...
| `AI_EXTRACTOR_MODELS` | `` | JSON array of AI-extractor model entries: `[{"id":"reader_lm","label":"reader-lm-0.5b","model":"jinaai/reader-lm-0.5b","baseUrl":"http://host.docker.internal:8000/v1"}]`. Each `id` becomes an "AI Model" option in the extractor dropdowns. Each entry may carry `"kind"`: `"chat"` (default — OpenAI-compatible `<baseUrl>/chat/completions`, `model` is the model name) or `"mineru"` (POST `{html}` to `<baseUrl>/extract`; e.g. `{"id":"mineru","label":"MinerU-HTML","model":"mineru","kind":"mineru","baseUrl":"http://navigator-mineru:8000"}`). Overrides `AI_EXTRACTOR_BASE_URL`+`AI_EXTRACTOR_MODEL`. `applies: "recreate"` — change requires a container restart. (Legacy name: `READER_LM_MODELS`) |
| `AI_EXTRACTOR_BASE_URL` | `` | Base URL of an OpenAI-compatible endpoint for the single-model AI extractor (e.g. `http://host.docker.internal:8000/v1`). Id defaults to the configured model name. (Legacy name: `READER_LM_BASE_URL`) |
| `AI_EXTRACTOR_MODEL` | `` | Model name sent to that endpoint (e.g. `jinaai/reader-lm-0.5b`). (Legacy name: `READER_LM_MODEL`) |
| `AI_EXTRACTOR_TIMEOUT_MS` | `60000` | Per-request timeout for the AI extractor. (Legacy name: `READER_LM_TIMEOUT_MS`) |
| `AI_EXTRACTOR_MAX_INPUT_CHARS` | `60000` | Max HTML chars sent to the model; longer HTML is **tail-cut** (the interesting content — tables, footnotes — lives at the end). (Legacy name: `READER_LM_MAX_INPUT_CHARS`) |
| `AI_EXTRACTOR_MAX_TOKENS` | `8192` | Max completion tokens requested. (Legacy name: `READER_LM_MAX_TOKENS`) |
