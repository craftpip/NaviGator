...
export const CONFIG_SCHEMA = [
  ...
  { key: "SEARCH_QUEUE_ERROR_GAP_PERCENTILE", category: "search", type: "number", fallback: 0.75, applies: "hot", description: "Failure-gap percentile used to learn safe retry intervals." },
  { key: "SEARCH_QUEUE_ERROR_GAP_SAFETY", category: "search", type: "number", fallback: 1.25, applies: "hot", description: "Safety multiplier applied to learned failure gaps." },
  { key: "SEARCH_QUEUE_DECAY_PER_SUCCESS", category: "search", type: "number", fallback: 0.75, applies: "hot", description: "Backoff multiplier after each successful recovery." },
  { key: "SEARCH_QUEUE_PROFILE_PATH", category: "search", type: "string", fallback: ".cache/search-engine-profiles.json", applies: "recreate", description: "Persisted scheduler profile path." },
  { key: "SEARCH_QUEUE_W_SUCCESS", category: "search", type: "number", fallback: 0.45, applies: "hot", description: "Scheduler score weight for success rate." },
  { key: "SEARCH_QUEUE_W_RESULTS", category: "search", type: "number", fallback: 0.15, applies: "hot", description: "Scheduler score weight for results per attempt." },
  { key: "SEARCH_QUEUE_W_STABILITY", category: "search", type: "number", fallback: 0.25, applies: "hot", description: "Scheduler score weight for recent stability." },
  { key: "SEARCH_QUEUE_W_RECENCY", category: "search", type: "number", fallback: 0.1, applies: "hot", description: "Scheduler score weight for recent-failure penalty." },
  { key: "SEARCH_QUEUE_W_RECOVERY", category: "search", type: "number", fallback: 0.05, applies: "hot", description: "Scheduler score weight for recovery successes." },
  { key: "SEARCH_QUEUE_W_LATENCY", category: "search", type: "number", fallback: 0.2, applies: "hot", description: "Scheduler score weight for observed response latency." },
  ...
  { key: "DEFAULT_EXTRACT_STABILIZE_STRATEGY", category: "ops", type: "enum", values: ["network_idle", "content_idle", "mutation", "none"], fallback: "network_idle", applies: "hot", description: "Default page stabilization strategy: hint/step settings win, else this (empty = network_idle)." },
  ...
  { key: "AI_EXTRACTOR_MODELS", category: "extractor", type: "string", fallback: "[]", applies: "recreate", description: 'JSON array of AI extractor models, e.g. [{"id":"reader_lm","label":"reader-lm-0.5b","model":"jinaai/reader-lm-0.5b","baseUrl":"http://host.docker.internal:8000/v1"}]. Each id becomes an "AI Model" option in the extractor dropdowns. Each entry also accepts an optional kind: "chat" (default; OpenAI-compatible /chat/completions, e.g. reader-lm) or "mineru" (POST {html} to baseUrl/extract, e.g. the MinerU-HTML v1.1 GPU sidecar). reader-lm weights are CC-BY-NC-4.0 non-commercial; the MinerU-HTML model carries the Tencent Hunyuan Community License.' },
  { key: "AI_EXTRACTOR_BASE_URL", category: "extractor", type: "string", fallback: "", applies: "recreate", description: "OpenAI-compatible base URL for the built-in AI extractor (e.g. http://localhost:11434/v1 or http://host.docker.internal:8000/v1). Empty disables the AI extractor. (Legacy name: READER_LM_BASE_URL.)" },
  { key: "AI_EXTRACTOR_MODEL", category: "extractor", type: "string", fallback: "reader-lm:0.5b", applies: "recreate", description: "Model id for the built-in AI extractor entry (vLLM would use jinaai/reader-lm-0.5b). (Legacy name: READER_LM_MODEL.)" },
  { key: "AI_EXTRACTOR_TIMEOUT_MS", category: "extractor", type: "number", fallback: 60000, applies: "hot", description: "Per-call timeout for the AI extractor (ms)." },
  { key: "AI_EXTRACTOR_MAX_INPUT_CHARS", category: "extractor", type: "number", fallback: 60000, applies: "hot", description: "Prepared HTML truncation for the AI extractor (~15K tokens)." },
  { key: "AI_EXTRACTOR_MAX_TOKENS", category: "extractor", type: "number", fallback: 8192, applies: "hot", description: "Generation token cap for the AI extractor." }
];
